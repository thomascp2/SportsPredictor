...
def safe_int(value, default=0):
    ...
    Args:
        value: Value to convert (can be None, int, float, string)
...
class NBAStatsAPI:
    ...
    def _make_request(self, endpoint, params):
        ...
        try:
            ...
            response = self.session.get(url, params=params, timeout=30)  # Increased from 10 to handle slow API
    ...
    def get_player_game_log(self, player_id, season=SEASON):
        ...
        for row in rows:
            ...
            logs.append({
                ...
                'minutes': safe_float(log.get('MIN')),
                'points': safe_int(log.get('PTS')),
                'rebounds': safe_int(log.get('REB')),
                'assists': safe_int(log.get('AST')),
                'steals': safe_int(log.get('STL')),
                'blocks': safe_int(log.get('BLK')),
                'turnovers': safe_int(log.get('TOV')),
                'threes_made': safe_int(log.get('FG3M')),
                'fgm': safe_int(log.get('FGM')),
                'fga': safe_int(log.get('FGA')),
                'ftm': safe_int(log.get('FTM')),
                'fta': safe_int(log.get('FTA')),
                'plus_minus': safe_int(log.get('PLUS_MINUS')),
            })
            ...
            if result_set.get('name') == 'PlayerStats':
                ...
                for row in rows:
                    ...
                    player_stats.append({
                        ...
                        'steals': safe_int(stats.get('STL')),
                        'blocks': safe_int(stats.get('BLK')),
                        'turnovers': safe_int(stats.get('TO')),
                        'threes_made': safe_int(stats.get('FG3M')),
                        'fgm': safe_int(stats.get('FGM')),
                        'fga': safe_int(stats.get('FGA')),
                        'ftm': safe_int(stats.get('FTM')),
                        'fta': safe_int(stats.get('FTA')),
                        'plus_minus': safe_int(stats.get('PLUS_MINUS')),
                    })
    ...
    def _parse_minutes(min_str):
        ...
        if not min_str or min_str == 'None' or min_str is None:
            return 0.0
        try:
