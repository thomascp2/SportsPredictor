...
def main():
    ...
    odds_data = fetch_and_save_odds("nhl", DB_PATH, game_date)
    ...
    for g in games:
        g["home_team"] = normalize_team("nhl", g["home_team"])
        g["away_team"] = normalize_team("nhl", g["away_team"])
    ...
    try:
        if send_game_predictions_alert("nhl", results):
